...
class BarCharPaymentNetworking extends StatefulWidget {
  const BarCharPaymentNetworking(
      {super.key,
  ...
  final List<PaymentNetworkingModel> dataMetric;

  @override
  State<BarCharPaymentNetworking> createState() =>
      _BarCharPaymentNetworkingState();
}

class _BarCharPaymentNetworkingState extends State<BarCharPaymentNetworking> {
  int touchedGroupIndex = -1;

  List<PaymentNetworkingModel> get currentPageData {
  ...
  List<BarChartGroupData> getData(double barsWidth, double barsSpace) {
    ...
    final List<PaymentNetworkingModel> currentPageData = this.currentPageData;

    for (int i = 0; i < currentPageData.length; i++) {
      final value = currentPageData[i].payments.amountOfPayments;
      final valueDoble = double.parse(value);
      final List<BarChartRodData> barRods = [
        BarChartRodData(
          color: currentPageData[i].eveEnd.isAfter(DateTime.now())
              ? Colors.grey
